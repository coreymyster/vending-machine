...
class MachineStatus:
    def __init__(self, label, prevBevStock, currentBevInStock, totalIncome):
        self.label = label
        self.prevBevStock = prevBevStock
        self.currentBevInStock = currentBevInStock
    ...
    def getPctSold(self):
        return (self.prevBevStock - self.currentBevInStock) / self.prevBevStock
    
    def getIncome(self):
        return self.totalIncome
    
    def __repr__(self):
        return self.label, self.prevBevStock, self.currentBevInStock, self.totalIncome
...
def main():
    ...
    for inventoryFileName in inventoryFileNames: 
        ...
        prevMachineStock = 0
        currentMachineStock = 0
        totalMachineIncome = 0
        ...
        for row in contents:
            ...
            for slot in row['slots']:
                ...
                # Calculates the stock for each machine and the income made off each machine
                currentMachineStock = currentMachineStock + slot['current_stock']
                prevMachineStock = prevMachineStock + slot['last_stock']
                totalMachineIncome = totalMachineIncome + ((slot['last_stock'] - slot['current_stock']) * slot['item_price'])
        
        # Updaqtes the vending machine dictionary and creates an object for each machine based on
        # values calculated above
        vendingMachineData[machineLabel] = MachineStatus(machineLabel, prevMachineStock, currentMachineStock, totalMachineIncome)
    ...
    machineStatusList = list(vendingMachineData.values())
    
    while reportChoice != 'q':
        reportChoice = input("Would you like the (m)achine report or the (i)nventory report? ")
        
        if reportChoice == 'm':
            print('Label           Pct Sold   Sales Total')
            for status in machineStatusList:
                print('{:10} {:12.2f}% ${:12}'.format(status.getLabel(), status.getPctSold() * 100, status.getIncome()))
        elif reportChoice == 'i':
            while sortChoice != 'q':
